...
namespace SocketTCP {
	...
	TEST_F(SocketCallsTest, SetSockOptions) {
		...
		u32 tv = 1;
		EXPECT_EQ(sockets->SetSockOptions(socket, SOL_SOCKET, SO_RCVTIMEO, reinterpret_cast<char*>(&tv), sizeof(tv)), 0);
	...
	TEST_F(SocketCallsTest, CheckRecv) {
		...
		EXPECT_EQ(calls.CheckRecv(socket, -1), RecvResult::kOk);
		WIN(WSASetLastError(WSAEWOULDBLOCK))NIX(errno = EAGAIN);
		EXPECT_EQ(calls.CheckRecv(socket, -1), RecvResult::kAgain);
		WIN(WSASetLastError(WSAECONNABORTED))NIX(errno = ECONNABORTED);
