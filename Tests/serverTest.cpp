...
namespace SocketTCP
{
	...
	namespace Server
	{
		...
		TEST_F(ServerTest, EnableKeepAliveWin)
		{
		...
		TEST_F(ServerTest, ErrSocket)
		{
			...
			WIN(WSASetLastError(1);)
			string err_message = std::format("Error: couldn't bind a socket ({}).", WIN(1) NIX(0));
			EXPECT_EQ(server_->explainStatus(), err_message);
			EXPECT_EQ(server_->start(), Server::Status::kErrSocketListening);
			WIN(WSASetLastError(1);)
			err_message = std::format("Error: couldn't listen a socket ({}).", WIN(1) NIX(0));
			EXPECT_EQ(server_->explainStatus(), err_message);
		}
		...
		TEST_F(ServerClientTest, CorrectProcess)
		{
			...
			EXPECT_CALL(socket_calls_mock, Recv(1, _, _, _))
				.Times(3)
				.WillOnce(Return(RecvResult::kOk))
